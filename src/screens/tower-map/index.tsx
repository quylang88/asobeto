...
import { ConnectionLinesSVG, FlyingStar, BossUnlockBurst, TowerNode } from "./components";
...
export function TowerSelection({
  ...
  const isPageSwitchDisabled = isUnlocking || showFlash || burstCenter !== null || flyingStars.length > 0;
  ...
  const handleBossUnlock = useCallback(() => {
    ...
    const STAR_COUNT = 6;
    const stars: FlyingStarData[] = Array.from({ length: STAR_COUNT }, (_, i) => {
      ...
      const midY =
        startY + (endY - startY) * (spread) - arcLift;
