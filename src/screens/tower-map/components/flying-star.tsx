...
function FlyingStarInner({
  ...
  const x = useTransform(progress, (p) =>
    bezierPoint(p, startX, midX, endX),
  );
  const y = useTransform(progress, (p) =>
    bezierPoint(p, startY, midY, endY),
  );

  /* Scale: start small, swell in the arc, settle at landing */
  const scale = useTransform(progress, [0, 0.15, 0.5, 0.85, 1], [0.3, 0.85, 1.15, 1.05, 0.7]);

  /* Opacity: fast fade‑in, hold, softer fade at the end */
  const opacity = useTransform(progress, [0, 0.08, 0.2, 0.88, 1], [0, 0.6, 1, 1, 0.6]);

  /* Gentle spin */
  const rotate = useTransform(progress, [0, 1], [0, 340 + Math.random() * 80]);

  /* Glow pulse intensity mapped to arc position */
  const glowOpacity = useTransform(progress, [0, 0.35, 0.6, 1], [0.3, 1, 0.85, 0.4]);
    ...
    <>
      ...
      {trails.map((dot) => (
        <motion.div
          ...
          initial={{ x: dot.x - 5, y: dot.y - 5, scale: dot.scale, opacity: dot.opacity }}
