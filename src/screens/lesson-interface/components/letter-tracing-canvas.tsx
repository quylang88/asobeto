...
export function LetterTracingCanvas({
  ...
  const activeStrokeRef = useRef<TraceStroke>([]);
  const drawnStrokesRef = useRef<TraceStroke[]>([]);
  ...
  const commitActiveStroke = () => {
    ...
    if (activeStroke.length >= 2) {
      drawnStrokesRef.current.push(activeStroke.slice());
    }
    activeStrokeRef.current = [];
  };
  ...
  // Chấm điểm theo độ khớp nét mờ + thứ tự nét đã vẽ
  const evaluateTrace = () => {
    ...
    const autoGeneratedDemoConfig =
      traceDemoStrategy === "auto" && isSingleLetterTarget
        ...
        : null;
    const scoringDemoConfig: TracingDemoAnimationConfig | undefined =
      traceDemoStrategy === "manual"
        ? traceDemoConfig
        : (autoGeneratedDemoConfig ?? undefined);
    ...
    onEvaluate(
      evaluateTracingScore({
        ...
        lesson,
        guideGlyphConfig,
        expectedStrokes: scoringDemoConfig?.strokes,
