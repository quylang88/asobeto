...
const VOWEL_DECOMPOSITION: Record<
  string,
  { base: string; tone: VietnameseTone }
> = {
  // A
  a: { base: "a", tone: "level" },
  á: { base: "a", tone: "sac" },
...
const CONSONANT_EQUIVALENTS: Record<string, string[]> = {
  ...
  d: ["gi", "r"],
  gi: ["d", "r"],
  r: ["d", "gi"], // Nhóm /z/ vs /j/ vs /r/ (Phương ngữ Nam/Bắc)
  s: ["x"],
  x: ["s"], // /s/ vs /ʂ/ (Phương ngữ)
  ch: ["tr"],
  tr: ["ch"], // /c/ vs /ʈ/ (Phương ngữ)
};
...
const TONE_SIMILARITY: Partial<
  Record<VietnameseTone, Partial<Record<VietnameseTone, number>>>
> = {
  level: {},
  huyen: {},
  sac: {},
