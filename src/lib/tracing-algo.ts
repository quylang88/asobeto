...
import { SOURCE_CANVAS_SIZE } from "@/data/tracing/utils";
    ...
    for (const neighborPixel of neighbors) {
      if (blockedPixel !== undefined) {
        if (typeof blockedPixel === "number") {
          ...
          if (blockedPixel.has(neighborPixel)) continue;
        }
      }
...
export function generateAutoDemoConfigFromGlyph(
  ...
  const autoStrokeCandidates = sortedComponents.flatMap((component) =>
    component.trails
      ...
      .filter(
        (candidate): candidate is GeneratedStrokeCandidate =>
          candidate !== null,
      ),
  );
